...
use crate::modules::files::{data_dir, ensure_config, config_path, load_app_settings, save_app_settings, load_app_settings_impl, save_app_settings_impl};
...
/// clear, actionable error. Bounded to ~30s so a genuinely broken startup
/// does not hang Commander indefinitely.
fn verify_named_wallet_loaded_after_start(wallet_name: &str) -> Result<(), String> {
  let wallet_arg = format!("-wallet={wallet_name}");
  // Give the daemon a moment to bind before the first probe.
  ...
  for _ in 0..40 {
    ...
    // If the daemon is up but the named wallet is not loaded, try
    // `loadwallet` once as a safe fallback (does not touch hemp.conf).
    if let Ok(raw) = crate::modules::commands::run_cli(&[String::from("listwallets")]) {
      if let Ok(loaded) = serde_json::from_str::<Vec<String>>(&raw) {
        if !loaded.iter().any(|w| w == wallet_name) {
          let _ = crate::modules::commands::run_cli(&[
            String::from("loadwallet"),
  ...
  Err(format!(
    "Core started, but the active vault wallet '{}' is not queryable after 30 seconds. Start Core through Commander again, or use the Wallet page to restore/load the vault wallet. No changes were made to hemp.conf.",
    wallet_name
  ))
}
...
pub fn wait_for_default_wallet_queryable(timeout: Duration) -> Result<(), String> {
  ...
  Err(format!(
    ...
    timeout.as_secs(),
    if last_error.is_empty() { "no wallet status returned" } else { last_error.as_str() }
  ))
}
    ...
    if let Ok(raw) = crate::modules::commands::run_cli(&[String::from("getblockchaininfo")]) {
      if serde_json::from_str::<serde_json::Value>(&raw).is_ok() {
        ...
        if let Some(wn) = wallet_name {
          ...
          if crate::modules::commands::run_cli(&[wallet_arg.clone(), String::from("getwalletinfo")]).is_ok() {
            return Ok(());
          }

          let load_args = vec![String::from("loadwallet"), wn.to_string()];
          match crate::modules::commands::run_cli(&load_args) {
            Ok(_) => {},
            Err(e) => {
              let lower = e.to_lowercase();
              if !lower.contains("already loaded") && !lower.contains("wallet already") {
                return Err(format!("Core is already running, but Commander could not load wallet '{}': {}", wn, e));
              }
            }
          }
          ...
          return Err(format!("Core is already running, but wallet '{}' is not queryable.", wn));
        }
        return Ok(());
      }
    }
...
pub async fn restart_node_with_wallet(wallet_name: String) -> Result<(), String> {
  ...
  tauri::async_runtime::spawn_blocking(move || {
    restart_node_with_wallet_robust(&wn, true)
  })
  .await
  .map_err(|e| format!("Restart task failed: {e}"))?
}

pub fn restart_node_with_wallet_robust(wallet_name: &str, poll_until_loaded: bool) -> Result<(), String> {
...
fn restart_node_with_wallet_robust_inner(
  ...
  if !force_restart && crate::modules::commands::run_cli(&[String::from("getblockchaininfo")]).is_ok() {
    let wallet_arg = format!("-wallet={wallet_name}");
    if crate::modules::commands::run_cli(&[wallet_arg.clone(), String::from("getwalletinfo")]).is_ok() {
      return Ok(());
    }
    ...
    match crate::modules::commands::run_cli(&load_args) {
      ...
      Err(e) => {
        let lower = e.to_lowercase();
        if !lower.contains("already loaded") && !lower.contains("wallet already") {
          // Continue to the restart path below; some Core states reject
          // loadwallet until after a clean restart.
        }
      }
    }
  }
...
pub fn set_network_mode(mode: String) -> Result<String, String> {
  ...
  if cfg_path.exists() {
    ...
    fs::copy(&cfg_path, &backup_path).map_err(|e| format!("Failed to create config backup: {}", e))?;
  }
  ...
  let trimmed = if trailing_newline { &content[..content.len() - 1] } else { &content };
  let trimmed = if trimmed.ends_with('\r') { &trimmed[..trimmed.len() - 1] } else { trimmed };
...
#[tauri::command]
pub async fn restore_legacy_wallet_dat(path: String, restart_node: bool) -> Result<serde_json::Value, String> {
  tauri::async_runtime::spawn_blocking(move || {
...
#[tauri::command]
pub async fn switch_to_legacy_wallet_dat(restart_node: Option<bool>) -> Result<serde_json::Value, String> {
  let do_restart = restart_node.unwrap_or(true);
  tauri::async_runtime::spawn_blocking(move || {
    switch_to_legacy_wallet_dat_blocking(do_restart)
  })
  .await
...
fn switch_to_legacy_wallet_dat_blocking(restart_node: bool) -> Result<serde_json::Value, String> {
  ...
  let wallet = dir.join("wallet.dat");
  if !wallet.exists() {
    return Err("No wallet.dat exists. Import a wallet.dat or create a new legacy wallet first.".to_string());
  }
...
fn restore_legacy_wallet_dat_blocking(path: String, restart_node: bool) -> Result<serde_json::Value, String> {
  let dir = data_dir()?;
  let wallet = dir.join("wallet.dat");
  let source = PathBuf::from(&path);
...
pub fn validate_wallet_file(path: String) -> Result<serde_json::Value, String> {
    ...
    use std::io::Read;
    file.read_exact(&mut buf).map_err(|e| format!("Cannot read file header: {e}"))?;

    let sqlite_header = b"SQLite format 3\0";
    let is_sqlite = buf.len() >= sqlite_header.len() && &buf[..sqlite_header.len()] == sqlite_header;
    if is_sqlite {
    ...
    let is_bdb = (buf.len() >= 16
        && (buf[12..16] == bdb_magic_be || buf[12..16] == bdb_magic_le))
        || (buf.len() >= 4
            && (buf[..4] == bdb_byteorder_be || buf[..4] == bdb_byteorder_le));
    ...
    Err("File does not appear to be a valid Core wallet file (expected BDB or SQLite format).".to_string())
}
...
mod tests {
  ...
  fn legacy_wallet_same_file_no_rename_or_delete() {
    ...
    let wallet_path = write_test_wallet(&tmp.path, "wallet.dat", b"SQLite format 3\0data-data-data");
    ...
    if let (Ok(src_canon), Ok(dst_canon)) = (source.canonicalize(), dest.canonicalize()) {
      assert_eq!(src_canon, dst_canon, "same file should canonicalize to same path");
    }

    // Verify the source file is untouched.
    let after_content = fs::read(&wallet_path).unwrap();
    assert_eq!(original_content, after_content, "same-file restore must not modify the source");
  ...
  fn legacy_wallet_different_file_copies_and_archives() {
    ...
    assert_eq!(fs::read(&source).unwrap(), b"new-wallet-content- SQLite format 3");

    // Destination should have new content
    let new_wallet = data_dir.join("wallet.dat");
    assert_eq!(fs::read(&new_wallet).unwrap(), b"new-wallet-content- SQLite format 3");

    // Backup should have old content
    assert!(backup.exists(), "backup should exist");
    assert_eq!(fs::read(&backup).unwrap(), b"old-wallet-content- SQLite format 3");
  }
  ...
  fn legacy_import_clears_active_vault_wallet_name() {
    use crate::modules::files::{TEST_COMMANDER_DIR, save_app_settings_impl, load_app_settings_impl};
    use crate::modules::models::AppSettings;

    let test_dir = std::env::temp_dir().join(format!("hemp64o_settings_clear_{}", std::process::id()));
    ...
    assert_eq!(loaded.active_vault_wallet_name, Some("hemp0x-vault-main".to_string()));
    ...
    let loaded = load_app_settings_impl().unwrap();
    assert!(loaded.active_vault_wallet_name.is_none(), "active_vault_wallet_name should be cleared after legacy import");
  ...
  fn switch_to_legacy_wallet_dat_clears_active_vault_without_restarting() {
    use crate::modules::files::{TEST_COMMANDER_DIR, TEST_DATA_DIR, save_app_settings_impl, load_app_settings_impl};
    use crate::modules::models::AppSettings;
